defmodule O2Gossip.APStations do
  ...
  def people_online(stations, interested, time_reference, threshold) do
    stations |> Enum.filter(fn(%{"Station MAC" => mac_address}) ->
      Enum.member?(interested, mac_address)
    end)
    ...
    |> Enum.map(fn(%{"Station MAC" => mac_address}) -> mac_address end)
  end
end
